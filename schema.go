...
func (o *Schema) validateSpec(loc string, opts *specValidationOptions) []*validationError {
	var errs []*validationError

	if o.Discriminator != nil {
		errs = append(errs, o.Discriminator.validateSpec(joinLoc(loc, "discriminator"), opts)...)
	}
	if o.XML != nil {
		errs = append(errs, o.XML.validateSpec(joinLoc(loc, "xml"), opts)...)
	}
	...
	if o.Not != nil {
		errs = append(errs, o.Not.validateSpec(joinLoc(loc, "not"), opts)...)
	}
	if o.AllOf != nil {
		for i, v := range o.AllOf {
			errs = append(errs, v.validateSpec(joinLoc(loc, "allOf", i), opts)...)
		}
	}
	if o.AnyOf != nil {
		for i, v := range o.AnyOf {
			errs = append(errs, v.validateSpec(joinLoc(loc, "anyOf", i), opts)...)
		}
	}
	if o.OneOf != nil {
		for i, v := range o.OneOf {
			errs = append(errs, v.validateSpec(joinLoc(loc, "oneOf", i), opts)...)
		}
	}

	// JsonSchemaCore
	if o.Schema != "" && o.Schema != Draft202012 {
		errs = append(errs, newValidationError(joinLoc(loc, "schema"), "must be '%s', but got '%s'", Draft202012, o.Schema))
	}
	if len(o.Defs) > 0 {
		for k, v := range o.Defs {
			errs = append(errs, v.validateSpec(joinLoc(loc, "defs", k), opts)...)
		}
	}
	if o.Type != nil {
		switch len(*o.Type) {
		...
		case 1:
			...
			default:
				errs = append(errs, newValidationError(joinLoc(loc, "type"), "must be one of [%s, %s, %s, %s, %s, %s, %s], but got '%s'", StringType, NumberType, IntegerType, BooleanType, ObjectType, ArrayType, NullType, v))
			}
		default:
			for i, v := range *o.Type {
				...
				case StringType, NumberType, IntegerType, BooleanType, ObjectType, ArrayType, NullType:
				default:
					errs = append(errs, newValidationError(joinLoc(loc, "type", i), "must be one of [%s, %s, %s, %s, %s, %s, %s], but got '%s'", StringType, NumberType, IntegerType, BooleanType, ObjectType, ArrayType, NullType, v))
				}
			}
		}
	}
	...
	if o.ContentSchema != nil {
		errs = append(errs, o.ContentSchema.validateSpec(joinLoc(loc, "contentSchema"), opts)...)
	}
	if o.ContentEncoding != "" {
		...
		default:
			errs = append(errs, newValidationError(joinLoc(loc, "contentEncoding"), "must be one of [%s, %s, %s, %s, %s, %s, %s], but got '%s'", SevenBitEncoding, EightBitEncoding, BinaryEncoding, QuotedPrintableEncoding, Base16Encoding, Base32Encoding, Base64Encoding, o.ContentEncoding))
		}
	}
	...
	if o.Default != nil {
		if !opts.doNotValidateDefaultValues {
			if e := opts.validator.ValidateDataAsJSON(loc, o.Default); e != nil {
				errs = append(errs, newValidationError(joinLoc(loc, "default"), e))
			}
		}
		if len(o.Enum) > 0 {
			var found bool
			...
			if !found {
				errs = append(errs, newValidationError(joinLoc(loc, "default"), "must be one of enum values: %v", o.Enum))
			}
		}
	}
	...
	if len(o.Examples) > 0 && !opts.doNotValidateExamples {
		for k, v := range o.Examples {
			if e := opts.validator.ValidateDataAsJSON(loc, v); e != nil {
	...
	if o.Type != nil {
		for _, t := range *o.Type {
			...
			case ArrayType: // JsonSchemaTypeArray
				if o.Items != nil {
					errs = append(errs, o.Items.validateSpec(joinLoc(loc, "items"), opts)...)
				}
				if o.MinItems != nil && *o.MinItems < 0 {
					errs = append(errs, newValidationError(joinLoc(loc, "minItems"), "must be greater than or equal to 0"))
				}
				if o.MaxItems != nil && *o.MaxItems < 0 {
					errs = append(errs, newValidationError(joinLoc(loc, "maxItems"), "must be greater than or equal to 0"))
					if o.MinItems != nil && *o.MaxItems < *o.MinItems {
						errs = append(errs, newValidationError(joinLoc(loc, "maxItems"), "must be greater than or equal to minItems"))
					}
				}
				if o.UnevaluatedItems != nil {
					errs = append(errs, o.UnevaluatedItems.validateSpec(joinLoc(loc, "unevaluatedItems"), opts)...)
				}
				if o.Contains != nil {
					errs = append(errs, o.Contains.validateSpec(joinLoc(loc, "contains"), opts)...)
				}
				if o.MinContains != nil && *o.MinContains < 0 {
					errs = append(errs, newValidationError(joinLoc(loc, "minContains"), "must be greater than or equal to 0"))
				}
				if o.MaxContains != nil && *o.MaxContains < 0 {
					errs = append(errs, newValidationError(joinLoc(loc, "maxContains"), "must be greater than or equal to 0"))
					if o.MinContains != nil && *o.MaxContains < *o.MinContains {
						errs = append(errs, newValidationError(joinLoc(loc, "maxContains"), "must be greater than or equal to minContains"))
					}
				}
				if len(o.PrefixItems) > 0 {
					for i, v := range o.PrefixItems {
						errs = append(errs, v.validateSpec(joinLoc(loc, "prefixItems", i), opts)...)
					}
				}
			case ObjectType: // JsonSchemaTypeObject
				if o.Properties != nil {
					for k, v := range o.Properties {
						errs = append(errs, v.validateSpec(joinLoc(loc, "properties", k), opts)...)
					}
				}
				if o.PatternProperties != nil {
					for k, v := range o.PatternProperties {
						errs = append(errs, v.validateSpec(joinLoc(loc, "patternProperties", k), opts)...)
						if _, err := regexp.Compile(k); err != nil {
							errs = append(errs, newValidationError(joinLoc(loc, "patternProperties", k), err))
						}
					}
				}
				if o.AdditionalProperties != nil {
					errs = append(errs, o.AdditionalProperties.validateSpec(joinLoc(loc, "additionalProperties"), opts)...)
				}
				if o.UnevaluatedItems != nil {
					errs = append(errs, o.UnevaluatedItems.validateSpec(joinLoc(loc, "unevaluatedItems"), opts)...)
				}
				if o.PropertyNames != nil {
					errs = append(errs, o.PropertyNames.validateSpec(joinLoc(loc, "propertyNames"), opts)...)
				}
				if o.MinProperties != nil && *o.MinProperties < 0 {
					errs = append(errs, newValidationError(joinLoc(loc, "minProperties"), "must be greater than or equal to 0"))
				}
				if o.MaxProperties != nil && *o.MaxProperties < 0 {
					errs = append(errs, newValidationError(joinLoc(loc, "maxProperties"), "must be greater than or equal to 0"))
					if o.MinProperties != nil && *o.MaxProperties < *o.MinProperties {
						errs = append(errs, newValidationError(joinLoc(loc, "maxProperties"), "must be greater than or equal to minProperties"))
					}
				}
				if len(o.Required) > 0 {
					for i, v := range o.Required {
						if _, ok := o.Properties[v]; !ok {
							errs = append(errs, newValidationError(joinLoc(loc, "required", i), "must be a property in properties"))
						}
					}
				}
			case NumberType, IntegerType: // JsonSchemaTypeNumber
				if o.MultipleOf != nil && *o.MultipleOf <= 0 {
					errs = append(errs, newValidationError(joinLoc(loc, "multipleOf"), "must be greater than 0"))
				}
				if o.Minimum != nil && *o.Minimum < 0 {
					errs = append(errs, newValidationError(joinLoc(loc, "minimum"), "must be greater than or equal to 0"))
				}
				if o.Maximum != nil && *o.Maximum < 0 {
					errs = append(errs, newValidationError(joinLoc(loc, "maximum"), "must be greater than or equal to 0"))
					if o.Minimum != nil && *o.Maximum < *o.Minimum {
						errs = append(errs, newValidationError(joinLoc(loc, "maximum"), "must be greater than or equal to minimum"))
					}
				}
				if o.ExclusiveMinimum != nil && *o.ExclusiveMinimum < 0 {
					errs = append(errs, newValidationError(joinLoc(loc, "exclusiveMinimum"), "must be greater than or equal to 0"))
				}
				if o.ExclusiveMaximum != nil && *o.ExclusiveMaximum < 0 {
					errs = append(errs, newValidationError(joinLoc(loc, "exclusiveMaximum"), "must be greater than or equal to 0"))
					if o.ExclusiveMinimum != nil && *o.ExclusiveMaximum < *o.ExclusiveMinimum {
						errs = append(errs, newValidationError(joinLoc(loc, "exclusiveMaximum"), "must be greater than or equal to exclusiveMinimum"))
					}
				}
				if o.Minimum != nil && o.ExclusiveMinimum != nil {
					errs = append(errs, newValidationError(joinLoc(loc, "minimum&exclusiveMinimum"), ErrMutuallyExclusive))
				}
				if o.Maximum != nil && o.ExclusiveMaximum != nil {
					errs = append(errs, newValidationError(joinLoc(loc, "maximum&exclusiveMaximum"), ErrMutuallyExclusive))
				}
			case StringType: // JsonSchemaTypeString
				if o.MinLength != nil && *o.MinLength < 0 {
					errs = append(errs, newValidationError(joinLoc(loc, "minLength"), "must be greater than or equal to 0"))
				}
				if o.MaxLength != nil && *o.MaxLength < 0 {
					errs = append(errs, newValidationError(joinLoc(loc, "maxLength"), "must be greater than or equal to 0"))
					if o.MinLength != nil && *o.MaxLength < *o.MinLength {
						errs = append(errs, newValidationError(joinLoc(loc, "maxLength"), "must be greater than or equal to minLength"))
					}
				}
				if o.Pattern != "" {
					if _, err := regexp.Compile(o.Pattern); err != nil {
						errs = append(errs, newValidationError(joinLoc(loc, "pattern"), err))
					}
				}
			}
		}
