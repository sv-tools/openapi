...
type Simple struct {
	Fs        string `json:"fs,omitempty" yaml:"FS,omitempty" openapi:",format:password"` // json name should be used
	Fi        int    `yaml:"FI,omitempty"`                                                // yaml name shuld be used
	Fb        *bool
	Fbs       []byte            `json:"fbs,omitempty" yaml:"FS,omitempty" openapi:"fBS"` // openapi name should be used
...
type SimpleByRef struct {
	S Simple `openapi:"s,required,title:Simple By Ref,ref:#/components/schemas/github.com.sv-tools.openapi_test.SimpleRef"`
}
...
func TestParseObject(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		},
		{
			...
			expected: openapi.NewSchemaBuilder().Ref("#/components/schemas/github.com.sv-tools.openapi_test.NoName").Build(),
			expectedComponents: openapi.NewComponents().Spec.Add(
				"github.com.sv-tools.openapi_test.NoName",
				openapi.NewSchemaBuilder().
					Type(openapi.ObjectType).
					AddProperty("Ref", openapi.NewSchemaBuilder().Ref("#/components/schemas/github.com.sv-tools.openapi_test.SimpleRef").Build()).
					GoPackage("github.com/sv-tools/openapi_test").GoType("openapi_test.Complex").Build(),
			).Add(
				"github.com.sv-tools.openapi_test.SimpleRef",
				openapi.NewSchemaBuilder().
					Type(openapi.ObjectType).
					AddProperty("fs", openapi.NewSchemaBuilder().Type(openapi.StringType).GoType("string").Format("password").Build()).
					AddProperty("FI", openapi.NewSchemaBuilder().Type(openapi.IntegerType).Format(openapi.Int64Format).GoType("int").Build()).
